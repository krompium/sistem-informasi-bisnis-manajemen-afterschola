...
> **Status:** Draft v1.2 (revisi konsistensi + keputusan NFR-4 — untuk dikonfirmasi ke Sabil sebelum dikunci)
> **Pemilik:** Sabil
> **Tanggal:** 2 September 2026 (revisi konsistensi: 15 September 2026; NFR-4: 16 September 2026)
> **Ruang lingkup dokumen:** MVP + arah pengembangan berikutnya
>
> **Perubahan v1.1 (konsistensi):**
> 1. Status absensi murid dikunci **boolean Hadir/Tidak Hadir** (§4.1, §9) — sebelumnya sempat tertulis H/I/S/A di dua tempat, kini konsisten dengan FR-5.1
> 2. FR-1.2 diperbaiki dari "3 role" jadi **5 role**, sesuai bagian lain dokumen
> 3. Dua bagian yang sama-sama bernomor **FR-7** dipisah: FR-7 (Laporan Ekspo) dan **FR-8** (Export) — FR-8/FR-9/FR-10 setelahnya ikut bergeser
> 4. FR-10 (Manajemen Proyek Developer) ditandai eksplisit **⏳ Fase 3, bukan MVP**, sesuai Fitur-per-Modul-After-Schola.md
> 5. Bukti kehadiran (foto/GPS) dipindah ke Fase 1 di Rencana Rilis, sesuai FR-6.4 yang sudah menyatakan ini masuk MVP
> 6. §2 diperbarui — klaim "belum dikonfirmasi" dihapus karena sudah tertutup di §10
>
> **Perubahan v1.2:**
> 7. NFR-4 (Audit trail) dihapus dari daftar NFR MVP — dipindah ke backlog v2,
>    menyamakan dengan Fitur-per-Modul-After-Schola.md yang menandainya `[v2]`.
>    Sebelumnya NFR-4 ada di §6 tanpa penanda fase, tersirat berlaku sejak MVP —
>    ini kontradiksi baru yang ketemu saat menyusun `tdd.md`, terpisah dari 6
>    kontradiksi v1.1 di atas.
>
> Perubahan di atas murni konsistensi, tidak mengubah keputusan yang sudah dibuat Sabil.
...
> Isi kolom "Modul utama" sudah dikonfirmasi — lihat §10 poin 1–2.
...
6. Tandai status tiap murid: **Hadir / Tidak Hadir** (checkbox per murid, default belum-hadir).
...
- FR-1.2 Role-based access control (RBAC) — 5 role (Management, Finance, HR, Trainer, Developer).
...
- FR-4.4 **Jadwal ngajar** = sesi per level (Beginner/Intermediate). **Ekspo/Free-Trial** ditangani sebagai **form laporan** terpisah (lihat FR-7 Laporan Ekspo), bukan sesi absensi murid.
...
### FR-8 Export Excel & PDF
- FR-8.1 Export rekap absensi murid (per kelas / per sekolah / per periode) ke **Excel**.
- FR-8.2 Export rekap ke **PDF** yang rapi & siap cetak (kop, nama sekolah, periode, tanda tangan).
- FR-8.3 Export rekap kehadiran trainer ke Excel/PDF.
- FR-8.4 Trainer hanya bisa export data kelas/sekolah miliknya.

### FR-9 Dashboard per Role
- FR-9.1 **Trainer:** sekolah yang dipegang, **jadwal ngajar & jadwal ekspo**, sesi hari ini, ringkas kehadiran kelasnya.
- FR-9.2 **Management:** total sekolah/murid/trainer, kehadiran hari ini lintas sekolah, trainer yang belum absen, grafik tren, + kelola user.

### FR-10 Modul Manajemen Proyek (Developer) — ⏳ Fase 3, BUKAN bagian MVP
> Dicatat di sini sebagai spesifikasi awal untuk referensi nanti. Jangan dibangun
> bersamaan dengan Modul Absensi — lihat Rencana Rilis §9.

Untuk tim teknis. Terdapat **beberapa team dev berbeda**; tiap team punya proyek & task sendiri.
- FR-10.1 Kelola **team dev** (beberapa team, tiap developer tergabung di satu/lebih team).
- FR-10.2 Kelola **proyek** per team (nama, deskripsi, status: perencanaan/berjalan/selesai, deadline).
- FR-10.3 Kelola **task** per proyek (judul, penanggung jawab/assignee, status, progres).
- FR-10.4 **Board task** gaya kanban: To Do → In Progress → Review → Done (drag/pindah kolom).
- FR-10.5 **Progres task**: status + persentase progres (atau checklist sub-task).
- FR-10.6 **Dashboard team dev**: proyek yang sedang berjalan, ringkas progres task, siapa mengerjakan apa.
- FR-10.7 Developer melihat board **team-nya**; Management melihat **semua team** (oversight).
...
> **NFR-4 (Audit trail) dipindah ke backlog v2**, sesuai
> `Fitur-per-Modul-After-Schola.md`. Nomor sengaja tidak diisi ulang supaya
> referensi NFR-5/NFR-6 di dokumen lain (`tdd.md`) tetap valid tanpa perlu
> disinkronkan ulang.
...
- Absensi murid (Hadir/Tidak Hadir) + absensi trainer (check-in, foto/screenshot + GPS — lihat FR-6.4)
...
- Jadwal sesi berulang (lihat FR-4.3), notifikasi
